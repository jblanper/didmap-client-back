...
class MapGraph(Graph):
    ...
    def _query_dbpedia(self, geonames_code: str, is_country: bool = False):
        ...
        try:
            ...
            if (is_country):
                query = sparql.setQuery("PREFIX owl: <http://www.w3.org/2002/07/owl#> PREFIX dbo: <http://dbpedia.org/ontology/> PREFIX dbp: <http://dbpedia.org/property/> SELECT ?country ?tel_code ?currency ?capital ?time_zone ?population (count(?wikilinks) as ?n_wikilinks) ?external_links WHERE { ?country owl:sameAs <http://sws.geonames.org/"+geonames_code+"/>; dbo:countryCode ?tel_code; dbp:capital [ rdfs:label ?cap ]; dbp:currency ?currency; dbp:timeZone ?time_zone; dbo:populationTotal ?population; dbo:wikiPageWikiLink ?wikilinks; dbo:wikiPageExternalLink ?external_links. FILTER(LANG(?cap) = 'es') }")
            else:
                query = sparql.setQuery("PREFIX owl: <http://www.w3.org/2002/07/owl#> PREFIX dbo: <http://dbpedia.org/ontology/> PREFIX dbp: <http://dbpedia.org/property/> SELECT ?continent ?time_z ?population (count(?wikilinks) as ?n_wikilinks) ?external_links WHERE { ?continent owl:sameAs <http://sws.geonames.org/"+geonames_code+"/>; dbp:time ?time_z; dbo:populationTotal ?population; dbo:wikiPageWikiLink ?wikilinks; dbo:wikiPageExternalLink ?external_links. }")
